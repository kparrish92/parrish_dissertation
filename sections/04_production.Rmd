...
In particular, it examines how Spanish-English bilinguals in both orders of acquisition produce German and French vowel and stop consonants.
...
The typological view, for example [see e.g. @rothmanrothman_l3_2011], suggests that overall crosslinguistic similarity will result in whole-language influence. 
...
While this view has been supported in syntax, it has not seen much empirical testing or support in phonology.
In reality, an increasing number of studies are finding that both languages impact third language phonological development. 
However, it is still not well understood what impacts the relative degree of influence of the L1 or L2 and whether, while they may not cause whole language influence, whether the factors of language status and typology may still be at play. 

In order to test this, the present study used a mirror-image design of participants and gave them words from two "third" languages produce (French and German).
...
Finally, views which suggest both languages impact L3 learning have not yet developed enough explanatory power to derive clear prediction related to Spanish-English bilinguals learning French and German.    
...
The pseudowords were comprised of the same vowel sounds tested in Chapter 3 in bilabial and fricative frames (/pVf/, /fVf/) and repeated 3 times each per frame. 
As a result, each participant produced 24 tokens in English and Spanish (4 vowels x 2 frames x 3 repetitions).
In the same screen, audio was submitted using a similar widget, which contains a button to record, playback, and submit.
The playback button allowed for participants to verify that they were submitting working audio.
The English vowels /a/, /i/, /o/, wedge were solicited via the following orthographic prompts: faff, fiff, foff, fuff.
...
### Word Repitition Task

In order to elicit French and German psuedowords, the same tokens used in the elicited production task in Chapter 3 (described in section X) were provided to participants to repeat. 
During a given trail, participants saw an orthographic representation of a pseudoword on their screen, together with a an audio widget which allowed the participants to play the audio production of the given word at will.
...
As a result, the French vowels wedge, /i/, /o/, and /y/ were represented in the psuedocarrier words: faf, fif, fof, fuf, paf, pif, pof, puf.
The German words German: wedge, i, o, y were elicited in the pseudocarrier words: faf, fif, fof, fuf
paf, pif, pof, puf.
...
The TPM is largely not supported by the preset data, since it would predict that in the case of a new sounds like /y/, the initial state of L3 acquisition is the final state of either a participants L1 or L2, and thus should be guided by L1 or L2 categories. 
...
Firstly, the LPM does not make specific predictions regarding sounds which are not present in the L1 or L2 in the L3, but would likely not predict successful imitation at first exposure, given that L1 and L2 categories are thought to impact the L3. 
...
For example, when examining L3 French production by Spanish-English bilinguals, Parrish (2022) only a subset of participants produced English and Spanish distinctly and were included in the study. 
...
Figures \@ref(fig:eng-part-one), \@ref(fig:eng-part-two) and \@ref(fig:eng-part-three) show the adjusted credible intervals for each L1 English speaker. 
In both, figure \@ref(fig:eng-part-one) and figure \@ref(fig:eng-part-three), it is plausible to conclude that French productions of these participants were mere Spanish-like, while German productions were more English-like.
On the other hand, figure \@ref(fig:eng-part-two) suggests that this participant did not produce distinct VOT as a function of language.

```{r, 'eng-part-one', echo=FALSE, fig.cap="Adjusted credible intervals of all plausible values of VOT as a function of Language for participant 492828"}
knitr::include_graphics(here("sections", "figs", "e_492828.png"))
```

```{r, 'eng-part-two', echo=FALSE, fig.cap="Adjusted credible intervals of all plausible values of VOT as a function of Language for participant 493072"}
knitr::include_graphics(here("sections", "figs", "e_493072.png"))
```

```{r, 'eng-part-three', echo=FALSE, fig.cap="Adjusted credible intervals of all plausible values of VOT as a function of Languagefor participant 500265"}
...
\@ref(fig:vod-mod) shows the adjusted posterior distributions for each language in the L1 Spanish group.
The range of plausible estimates for both French and German fell between English and Spanish, with German being closer to English than French.

```{r, 'vod-mod', echo=FALSE, fig.cap="Adjusted credible intervals of all plausible values of VOT as a function of Language"}
knitr::include_graphics(here("sections", "figs", "vot_mod.png"))
```

# Discussion 

These results replicated previous work (Parrish 2022), which found that L3 French VOT was intermediate of L1 and L2 VOT when those languages are Spanish and English.
...
The German results were higher on average than French, but still fell intermediate of L1 and L2 values in the L1 Spanish group.
...
At the same time, the results for VOT and vowels were distinct. 
Largely, successful imitation was obtained for novel vowel formants, while VOT was far less successful.
There are at least three possible reasons for this finding.
First, it is possible that, since the VOT of stop consonants is less salient than vowels quality, that VOT distinctions fall below the level of consciousness and are therefor harder to imitate at first exposure. 
Secondly, task demands may have impacted the results.
The fact the multiple vowel conditions and a rather long production task were utilized might introduced a heavy processing load in participants that made it too difficult to attend to VOT.
...
Just as with vowels, the present data on stops informs L3 models. 
If the VOT data were taken alone, the LPM best fits the data, since it explicitly predicts the L3 values will fall intermediate of L1 and L2 values.
The TPM and L2 Status Factor cannot readily explain intermediate VOT that has been found here or in previous research, since these models would predict whole language influence of either the typogically closer language or the second learned language respectively.
...
The purpose of including these participants was to examine the starting point of L3 phonology.
Future research could benefit from using a longitudenal design and multiple segments to more fully support that idea that L3 learners are differently impacted by the L1 and L2 depending upon the segment and how this may change during L3 development.
...
Future work should consider the role of the segment, stage of development, and typological similarity of the involved languages as subjects that may impact L3 production.f
