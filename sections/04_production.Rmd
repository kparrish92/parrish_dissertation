...
The typological view, for example, suggests that overall crosslinguistic similariy will result in whole-language influence. 
...
If the groups perform differently, it could provide evidence that order of acquisition impacts access to linguistic representations in new language learning. 
...
A total of X participants participated in the present experiments and made up a total of 4 groups: L1 English bilingual (n = `r nrow(unique(full_data %>% filter(group == "L1 English bilingual") %>% select(prolific_id)))`), L1 Spanish bilingual (n = `r nrow(unique(full_data %>% filter(group == "L1 Spanish bilingual") %>% select(prolific_id)))`), L1 English monolingual (n = `r nrow(unique(full_data %>% filter(group == "L1 English monolingual") %>% select(prolific_id)))`) and L1 Spanish monolingual (n = `r nrow(unique(full_data %>% filter(group == "L1 Spanish monolingual") %>% select(prolific_id)))`).
