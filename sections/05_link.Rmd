...
Learning to speak a language necessarily entails both effectively perceiving and producing it.
In first language acquisition, there is evidence that perceptual abilities precede those of production.
For example, there is evidence that infants are born with the ability to discriminate any of the world's language contrasts [@eimas1971speech], and that language-specific sound discrimination occurs between 6-12 months of age [@kuhl2006infants], while production emerges later, beginning with a babbling stage around 6 months of age [@majorano2014relationship]. In L2 learning, it has also been debated whether perceptual learning occurs prior to production, in which there is less agreement overall.
...
This finding has received empirical support in several language pairings.
...
It has also been found that the relationship between production and perception is segment-dependent or are autonomously represented altogether.
...
Given the history of contradictory results regarding whether the development of production is dependent upon production, more recent work has aimed to identify and rectify the issues that may have contributed to the conflicting results.
...
Another recent longitudinal study, @nagle2018examining also examined L2 Spanish stop production over 4 semesters of non-immersion Spanish courses.
...
Models of L2 acquisition predict also consider a role for the link between perception and production.
The PAM and PAM-L2 [@bohn_nonnative_2007], based on DRT, suggest that the perception of sound contrasts predicts the acquisition of new-language sounds.
The original SLM [@flege1995second] also theorized that accurate perception preceded production also.
However, in a new revision, The SLM-R [@wayland_revised_2021] now predicts that non-native sounds develop in tandem, rather than perception preceding production.
...
On the other hand, property-by-property models would more readily explain a difference between perception and production, since they suggest that it is possible to partially access a source language as a source of influence.

Only one study examines the perception-production link in L3 acquisition [@wrembel2022relationship].
...
To measure perception, the authors used a forced-choice goodness task, in which participants heard two versions of a target word within a frame and chose which was a better exemplar.
...
Since the perception data was categorical forced-choice categorization data and the production data was continuous, the production data was converted from continuous to categorical using a method devised by the author for the purpose of the present analysis using an R script.
...
From here, using the probability derived from the Euclidian distance of each data point, Spanish (coded a 1) and English (coded as 0) "choices" were probabilistically simulated using a function in R.
...
The corresponding centroids were /i/ in all languages, /y/ and /u/ in French and German and Spanish and English, /o/ in French, German and Spanish to English /\textscripta/,  and /\textturnv/ in French German and English to Spanish /a/. 
Table \ref{labels_table} shows each corresponding phoneme and its label in the data.
...
To analyze the newly formed data set, Bayesian logistic regression models were run for each the Spanish L1 and English L1 group.
In each case, the outcome variable was binary: choosing a Spanish category (coded as 1) or an English category (coded as 0).
...
An overall bias for English was evident for /i/, /o/ and /\textturnv/, but the production of /y/ was assimilated to /u/ in both languages similarly. 
...
A post-hoc explanation from the LPM might be that modality (perception versus production) has an impact on this full transfer potential, where access to a phonetic category in perception is initially easier than in production.
